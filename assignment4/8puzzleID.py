...
def iddfs(initial, goal):
    depth_limit = 0
    while True:
        result = dfs_limit(initial, goal, depth_limit)
        if result:
            return result
        depth_limit += 1

def dfs_limit(initial, goal, depth_limit, path=[]):
    if depth_limit == 0:
        return None
    ...
    for action in possible_actions(initial):
        ...
        if successor_state not in path:
            new_path = dfs_limit(successor_state, goal, depth_limit - 1, path + [successor_state])
            if new_path:
                return new_path
...
sol_path = iddfs(init_state, goal_state)
