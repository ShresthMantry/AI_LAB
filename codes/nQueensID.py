...
def solve_n_queens_util(board, col, N, result, max_depth):
    if col == N:
    ...
    for i in range(N):
        if is_safe(board, i, col, N):
            board[i][col] = 1
            solve_n_queens_util(board, col + 1, N, result, max_depth - 1)
...
def solve_n_queens(N):
    ...
    max_depth = 1
    while not solve_n_queens_util(board, 0, N, result, max_depth):
        max_depth += 1
        board = [[0]*N for _ in range(N)]  # Clear the board for the next iteration
